...
const ItemConfigurableView = (props) => {
    ...
    return (
        <CustomRadio
            ...
            {...other}
            className="w-full overflow-x-scroll scrollbar-none !mb-0 items-center"
            label={false}
            classNames={{
                radioGroupClasses: classNames('!flex-row w-max gap-0.5 !mt-0', error && Object.keys(error).length > 0 && 'mb-1'),
            }}
        />
    );
};
